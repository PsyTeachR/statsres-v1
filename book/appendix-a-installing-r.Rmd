...
In chapter 10, you will use the `r pkg("brms")` to fit Bayesian regression models. Behind the scenes, this package uses a programming language called <a href="https://mc-stan.org/" target="_blank">Stan</a> which requires a C++ compiler. Unlike other packages, you cannot just use the `r hl(install.packages())` function to install it, there are a few steps to follow.
...
Once you have the program relevant to your computer, then you can install `r pkg("brms")` as usual: 
